...
| 7.2 | Hosted preview (Vercel + GitHub `Preview` environment) | ⛔ | `pre-prod-gate.yml` fails at step *Guard — secrets and schema pin*: the `Preview` environment has no `PREVIEW_DATABASE_URL` (run `35982950166`, 11 s). Adding it requires the hosted credential, which is not present in this workspace and must not be pasted into chat. Hosted `visa_os_preview` therefore still carries migrations ≤ 0012 |
