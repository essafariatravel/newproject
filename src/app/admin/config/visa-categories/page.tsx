...
export default async function VisaCategoriesConfigPage({
  ...
  return (
    <>
      <PageHeader title="Visa categories" subtitle="Top-level visa families: tourist, business, student…" />
      <Flash {...flash} />
      ...
      <TableWrap>
        ...
        <tbody className="divide-y divide-slate-100">
          ...
          {rows.length === 0 ? (
            <tr><td colSpan={5} className="td py-8 text-center text-slate-500">No categories configured.</td></tr>
          ) : null}
        </tbody>
      </TableWrap>
      ...
      {canManage ? (
        <div className="mt-8">
          <h2 className="mb-3 font-serif text-xl text-navy-900">Add category</h2>
          <form action={createVisaCategoryAction} className="card grid grid-cols-1 gap-4 p-5 sm:grid-cols-4">
            ...
            <div className="sm:col-span-4">
              <SubmitButton className="btn-primary" pendingLabel="Saving…">Add category</SubmitButton>
            </div>
          </form>
        </div>
      ) : null}
