...
export default async function CurrenciesConfigPage({
  ...
  return (
    <>
      <PageHeader title={ct("Currencies")} subtitle={ct("Supported fee and wallet currencies.")} />
      <Flash {...flash} />
      ...
      <TableWrap>
        ...
        <tbody className="divide-y divide-slate-100">
          ...
          {rows.length === 0 ? (
            <tr><td colSpan={5} className="td py-8 text-center text-slate-500">{ct("No currencies configured.")}</td></tr>
          ) : null}
        </tbody>
      </TableWrap>
      ...
      {canManage ? (
        <div className="mt-8">
          <h2 className="mb-3 font-serif text-xl text-navy-900">{ct("Add currency")}</h2>
          <form action={createCurrencyAction} className="card grid grid-cols-1 gap-4 p-5 sm:grid-cols-4">
            ...
            <div>
              <label className="label" htmlFor="symbol">{ct("Symbol *")}</label>
              <input id="symbol" name="symbol" required maxLength={8} className="input" placeholder="CHF" />
            </div>
            <div className="sm:col-span-4">
              <SubmitButton className="btn-primary" pendingLabel={ct("Saving…")}>{ct("Add currency")}</SubmitButton>
            </div>
          </form>
        </div>
      ) : null}
